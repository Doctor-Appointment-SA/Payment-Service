...
- **รายการชำระเงินหมดอายุอัตโนมัติใน 60 วินาที** — เมื่อสร้างรายการชำระเงินแล้ว ระบบจะนับถอยหลัง
  ถ้าครบ 60 วินาทีแล้วยังเป็น `PENDING` อยู่ รายการนั้นจะถูก **ลบทิ้ง** (ไม่ใช่เปลี่ยนสถานะเป็น `CANCEL`)
- **ติดตามการจัดส่งยา** — `PREPARE` → `SENDING` → `SUCCESS`
  (สถานะจะเดินหน้าอัตโนมัติด้วยตัวจำลอง ยังไม่ได้เชื่อมกับระบบขนส่งจริง)
- **อัปเดตแบบ delivery แบบ real-time ด้วย SSE** — frontend เปิด connection ค้างไว้
  แล้ว backend จะบอกไปยัง client เมื่อสถานะมีการเปลี่ยนแปลง 
...
payment จะถูกสร้างขึ้นเมื่อกดยืนยันการสั่งยา หลังจากนั้นแสดง qrcode สำหรับชำระเงิน
ถ้าระหว่างนี้ payment ไม่ถูกชำระใน 60 วินาที payment จะถูกยกเลิกและต้องยืนยันการสั่งยาใหม่
api เส้นนี้นับเวลาถอยหลัง หากครบ 60 วินาที จะทำการ cancel payment  
...
ระบบ delivery มีการ update แบบ real-time ผ่าน UI 
แต่การ update เป็นตัวจำลองที่จะเดินสถานะให้เอง: `PREPARE` → `SENDING` (หลัง 10 วินาที) → `SUCCESS` (หลัง 20 วินาที)
